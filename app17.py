...
ktp_cetak = st.text_input(
    "Masukkan Nomor KTP yang Terdaftar"
)

if st.button("Cetak Tiket"):

    data_tiket = st.session_state.tiket.cari_ktp(
        ktp_cetak
    )
    ...
    if data_tiket:
        ...
        kode_tiket = (
            f"JB-{data_tiket.nama[:3].upper()}-{data_tiket.ktp[-4:]}"
        )
...
Email           : {data_tiket.email}
No Telepon      : {data_tiket.telepon}
No KTP          : {data_tiket.ktp}

Tanggal Lahir   : {data_tiket.tanggal_lahir}

Kategori Tiket  : {data_tiket.kategori}
Jumlah Tiket    : {data_tiket.jumlah}

Harga Tiket     : Rp {data_tiket.harga:,}
